                  ...
                  data?.items.map((item) => (
                    <TableRow key={String(item.id)}>
                      {config.columns.map((column) => (
                        <TableCell key={column.key}>{renderValue(getPathValue(item, column.key))}</TableCell>
                      ))}
                      <TableCell>
          ...
          <DialogBody>
            ...
            <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit(submit)}>
              {config.fields.map((field) => (
                <Field key={field.name} field={field} register={register} errors={errors} options={options[field.source ?? ''] ?? field.options ?? []} watch={watch} />
              ))}
...
function buildSchema(fields: FieldConfig[]) {
  ...
  for (const field of fields) {
    switch (field.type) {
      case 'text':
      case 'textarea':
        shape[field.name] = field.required ? z.string().min(1, `${field.label} is required`) : z.string().optional().or(z.literal(''));
        break;
      case 'number':
        if (field.required) {
          shape[field.name] = z.string().min(1, `${field.label} is required`).refine(val => !Number.isNaN(Number(val)), `${field.label} must be a valid number`);
        } else {
          shape[field.name] = z.string().optional().or(z.literal('')).refine(val => val === undefined || val === '' || !Number.isNaN(Number(val)), `${field.label} must be a valid number`);
        }
        break;
      case 'date':
        shape[field.name] = field.required ? z.string().min(1, `${field.label} is required`) : z.string().optional().or(z.literal(''));
...
function renderValue(value: any) {
  if (typeof value === 'boolean') {
    return <Badge className={value ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}>{value ? 'Active' : 'Inactive'}</Badge>;
  }
