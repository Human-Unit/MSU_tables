...
func (s *ScheduleService) Weekly(ctx context.Context, facultyID, vocationID, educationYear, groupID string) ([]map[string]any, error) {
	...
	if groupID != "" {
		...
		if needsGroupJoin {
			query = query.Joins(`JOIN "group" ON "group".id = schedule.group_id`)
		}
		if educationYear != "" {
			if year, err := strconv.Atoi(educationYear); err == nil {
				query = query.Where(`"group".education_year = ?`, year)
			}
		}
		if vocationID != "" {
			query = query.Where(`"group".vocation_id = ?`, vocationID)
		} else if facultyID != "" {
			query = query.Joins(`JOIN vocation ON vocation.id = "group".vocation_id`).
