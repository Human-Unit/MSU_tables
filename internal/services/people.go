...
func listStudentRows(ctx context.Context, db *gorm.DB, search string, page, pageSize int) (models.PageResult, error) {
	...
	query := db.WithContext(ctx).Table("person").
		...
		Joins("JOIN groups ON groups.id = student_profile.group_id").
	...
	if trimmed := strings.TrimSpace(search); trimmed != "" {
		...
		query = query.Where(
			"LOWER(person.full_name) LIKE ? OR LOWER(person.email) LIKE ? OR LOWER(groups.name) LIKE ? OR LOWER(users.username) LIKE ?",
